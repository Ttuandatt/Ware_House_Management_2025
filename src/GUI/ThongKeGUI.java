...
public class ThongKeGUI extends JPanel {
	...
	public void initLeftPanel() {
		...
		importChartPanel = createPieChart("IMPORT", 1);
		exportChartPanel = createPieChart("EXPORT", 2);
		...
		String[] optionData = { "Import", "Export" };
...
tablePanel.revalidate();
						...
						khoData[0] = "All warehouses    ";
						int i = 1;
						int j = 0;
						while (j < list.size()) {
							khoData[i++] = list.get(j++);
						}
						for(String s: khoData) {
							khoComboBox.addItem(s);
						}
					...
					if (leftPanel.isAncestorOf(importChartPanel)) {
						...
						tablePanel.repaint();
						ArrayList<String> list = new PhieuXuatBUS().getAllMakho();
						String[] khoData = new String[list.size() + 1];
						khoComboBox.removeAllItems();
		...
		JLabel optionLabel = new JLabel("Statistics option:");
		...
		JLabel startLabel = new JLabel("Begin date:");
...
//					importChartPanel = createPieChart("EXPORT", result);
...
arr = new PhieuXuatBUS().getByDate(endData, "<");
	...
	public void initRightPanel() {
		...
		JLabel khoLabel = new JLabel("Warehouse:");
		ArrayList<String> list = new PhieuNhapBUS().getAllMakho();
		String[] khoData = new String[list.size() + 1];
		khoData[0] = "All warehouses";
		int i = 1;
...
DefaultTableModel model = (DefaultTableModel) exportTable.getModel();
	...
	public void initImportTable() {
		...
		String[] columnData = { "Note ID", "Creator ID ", "Warehouse ID", "Supplier ID", "Dâte", "Total" };
...
JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
	...
	public void initExportTable() {
		...
		String[] columnData = { "Note ID", "Creator ID ", "Warehouse ID", "Customer ID", "Date", "Total" };
		TableModel tableModel = new DefaultTableModel(columnData, 30);
