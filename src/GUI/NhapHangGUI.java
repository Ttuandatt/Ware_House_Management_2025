...
// North
		...
		JLabel codeLabel = new JLabel("Note ID:                ");
		...
		JLabel khoLabel = new JLabel("Warehouse ID:      ");
...
//		providerPanel.setLayout(new BorderLayout(47, 0));
		JLabel providerLabel = new JLabel("Supplier:          ");
		...
		JLabel creatorLabel = new JLabel("Creator ID:            ");
...
//Center
		...
		String[] columnData = { "Product ID", "Product ID", "Quantity", "Price" };
		...
		JButton quantityButton = new JButton("Fix quantity");
		...
		JButton deleteButton = new JButton("Remove product");
...
//		
	...
	public void changeQuantity(JButton btn, JTable table) {
		...
		if (selectedRow == -1) {
			JOptionPane.showMessageDialog(null, "Please select a product to fix quantity");
		} else {
			DefaultTableModel model = (DefaultTableModel) table.getModel();
			String productCodeString = (String) model.getValueAt(selectedRow, 0);
			String newQuantity = JOptionPane.showInputDialog("Please enter quantity for " + productCodeString);
	...
	public void initLeftPanel() {
		...
		productPanel.setBorder(BorderFactory.createTitledBorder("Product list"));
		ArrayList<SanPhamDTO> listSanPham = new SanPhamBUS().getAllSanPham();

		String[] columnData = { "Product ID", "Product Name", "Quantity", "Price" };
		TableModel tableModel = new DefaultTableModel(columnData, 30);
		...
		searchPanel.setBorder(BorderFactory.createTitledBorder("Searching"));
		...
		actionMap.put(enterStroke.toString(), new AbstractAction() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if (searchTextArea.equals("")) {
					...
					optionMap.put("ID", "masp");
					optionMap.put("Name","tensp");
					optionMap.put("Price > value",">");
					optionMap.put("Price < value","<");
					optionMap.put("Price = value","=");
					ArrayList<SanPhamDTO> listSP ;
					if(condition.equals("Price > value")||condition.equals("Price < value")||condition.equals("Price = value" )){
		...
		String[] searchLabel = { "ID", "Name","Price > value","Price < value","Price = value" };
		...
		JButton addQuantityButton = new JButton("OK");
		...
		JButton addNewButton= new JButton("New product");
		addNewButton.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				new PopupSanPham("Add new product","Add product", "add");
			}
		});
...
//			btn.setForeground(Color.decode("#F0F3FF"));
	...
	public void addToImportTable(JTable table, JTable importTable, JTextArea quantity) {
		...
		if (quantity.getText().equals("")) {
			JOptionPane.showMessageDialog(null, "Plase enter quantity");
			return;
	...
	public void updateSoLuong(JTable table) {
		...
		JOptionPane.showMessageDialog(null, tbao);
			...
			JOptionPane.showMessageDialog(null, "Product doesn't exist in added table");
		}
