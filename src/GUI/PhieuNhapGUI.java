...
public class PhieuNhapGUI extends JPanel {
	...
	public void initLeftPanel() {
		...
		String[] searchLabel = { "ID", "Creator ID", "Supplier ID", "Price < value", "Price = value",
				"Price > value", "Date > value", "Date < value", "Date = value" };
		...
		ArrayList<PhieuNhapDTO> listCT = new PhieuNhapBUS().getAllPhieuNhap();
		String[] columnData = { "Note ID", "Creator ID", "Warehouse ID", "Supplier ID", "Date", "Total" };
		...
		for (int i = 0; i < listCT.size(); i++) {
			...
			String ngaytao = listCT.get(i).getNgaytao().toString();
			String tongtien = NumberFormat.getInstance().format(listCT.get(i).getTongtien()) + "đ";
			model.addRow(new Object[] { mapn,  mangtao,makho, mancc, ngaytao, tongtien });
		}
	...
	public void initRightPanel() {
		...
		searchDetailPanel.setBorder(BorderFactory.createTitledBorder("Searching"));
		String[] searchDetailData = { "Product ID", "Product name","Quantity = value","Quantity > value","Quantity < value", "Price = value","Price < value","Price > value"};
		...
		String[] columnData = { "Note ID", "Product ID", "Product name", "Quantity", "Total" };
	...
	public void viewDetail() {
		...
		if (selectedRow != -1) {
			...
			JOptionPane.showMessageDialog(null, "Please select a note to view");
		}
	}
	...
	public void searchImport() {
		...
		condition.put("Note ID", "mapn");
		condition.put("Creator ID", "manguoitao");
		condition.put("Supplier ID", "mancc");
		condition.put("Price = value", "=");
		condition.put("Price < value", "<");
		condition.put("Price > value", ">");
		condition.put("Date > value", ">");
		condition.put("Date < value", "<");
		condition.put("Date = value", "=");
		if (option.indexOf("ID") != -1) {

			arr = new PhieuNhapBUS().getByCondition(data, condition.get(option));
		} else if (option.indexOf("Price") != -1) {
			...
			if (data.matches("\\d{4}-\\d{2}-\\d{2}")==false) {
				JOptionPane.showMessageDialog(null, "Required format: YYYY-MM-DD");
				return;
			}
	...
	public void searchDetail() {
		...
		HashMap<String, String> condition = new HashMap<String, String>();
		condition.put("Note ID", "mapn");
		condition.put("Product ID", "masp");
		condition.put("Product name", "tensp");
		condition.put("Price = value", "=");
		condition.put("Price < value", "<");
		condition.put("Price > value", ">");
		condition.put("Quantity = value", "=");
		condition.put("Quantity < value", "<");
		condition.put("Quantity > value", ">");
		...
		}else if(option.indexOf("Quantity")!=-1) {
