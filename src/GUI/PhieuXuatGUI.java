...
public class PhieuXuatGUI extends JPanel {
    ...
    private void initComponents() {
        ...
        searchPanelLeft.setBorder(BorderFactory.createTitledBorder("Searching"));
        ...
        tfSearchLeft.setBounds(5, 15, 190, 30);
        JButton searchButtonLeft = new JButton("Search");
        searchButtonLeft.setBounds(279, 15, 75, 30);
        ...
        JButton refreshButtonLeft = new JButton("Refresh");
        refreshButtonLeft.setBounds(5, 15, 85, 30);
        JButton excelButtonLeft = new JButton("Excel");
        excelButtonLeft.setBounds(90, 15, 90, 30);
        JButton removeButtonLeft = new JButton("Remove");
        removeButtonLeft.setBounds(200, 15, 78, 30);
        
     
        
        optionButtonPanelLeft.add(refreshButtonLeft);
        optionButtonPanelLeft.add(excelButtonLeft);
        searchBoxPanelLeft.add(removeButtonLeft);
        ...
        removeButtonLeft.addMouseListener(new MouseAdapter(){
            @Override
            public void mouseEntered(MouseEvent e){
                removeButtonLeft.setCursor(new Cursor(Cursor.HAND_CURSOR));
                removeButtonLeft.setBackground(Color.decode("#D6D6D6"));
            }
            @Override
            public void mouseExited(MouseEvent e){
                removeButtonLeft.setBackground(Color.WHITE);
            }
        });
        
        removeButtonLeft.addActionListener(new ActionListener(){
            @Override
            public void actionPerformed(ActionEvent e){
                xoaPerformed(exportTable);
            }
        });
...
//        
        ...
        excelButtonLeft.addActionListener(new ActionListener(){
            @Override
            public void actionPerformed(ActionEvent e){
        ...
        searchPanelRight.setBorder(BorderFactory.createTitledBorder("Searching"));
        ...
        tfSearchRight.setBounds(5, 15, 250, 30);
        JButton searchButtonRight = new JButton("Search");
        searchButtonRight.setBounds(260, 15, 80, 30);
        searchBoxPanelRight.add(tfSearchRight);
        searchBoxPanelRight.add(searchButtonRight);

        // Tạo và thêm các nút "Tìm" và "Làm mới" trong searchButtonPanelLeft
        JButton refreshButtonRight = new JButton("Refresh");
        refreshButtonRight.setBounds(0, 15, 85, 30);
        JButton excelButtonRight = new JButton("Excel");
...
//        loadExportDetailList(exportTable);
    ...
    private void loadExportList() {
        ...
        exportModel.addColumn("Note ID");
        exportModel.addColumn("Creator ID");
        exportModel.addColumn("Warehouse ID");
        exportModel.addColumn("Customer ID");
        exportModel.addColumn("Date");
        exportModel.addColumn("Total");
    ...
    private void loadExportDetailList(JTable tb){
        ...
        exportDetailModel.addColumn("Note ID");
        exportDetailModel.addColumn("Product ID");
        exportDetailModel.addColumn("Product name");
        exportDetailModel.addColumn("Quantity");
        exportDetailModel.addColumn("Price");
    ...
    private void searchPerformed(JTable exportTable){
        ...
        if(!searchContent.isEmpty()){
            ...
            if(!found){
                JOptionPane.showMessageDialog(this, "Cannot find delivery note");
                lamMoi();
                return;
            }
    ...
    public void xoaPerformed(JTable tb){
        int selectedRow = tb.getSelectedRow();
        if(selectedRow!=-1){
            int dialogResult = JOptionPane.showConfirmDialog(null,"Are you sure want to remove?", "Confirm", JOptionPane.OK_CANCEL_OPTION);
            if(dialogResult == JOptionPane.OK_OPTION){
                String mapx = (String)exportModel.getValueAt(selectedRow, 0);
                String message = pxBUS.deletePhieuXuat(mapx);
                if(message.equals("Remove delivery note succefully!")){
                    exportModel.removeRow(selectedRow);
                    arrPX.remove(selectedRow);
                }
                JOptionPane.showMessageDialog(this, message);
            }
        }else {
            JOptionPane.showMessageDialog(null, "Please select a note to remove.", "Warning", JOptionPane.WARNING_MESSAGE);
        }
    }
